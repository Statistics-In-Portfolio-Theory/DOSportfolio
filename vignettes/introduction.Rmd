...
data <- 5/3 * matrix(rt(n*p, df=5), ncol=p, nrow=n) %*% t(chol(Sigma))
```
...
```
The issue concerns what is called the "concentration ratio" which we denote $c_i=p/n_i$, where $n_i$ is the size of each subsample. In our case these are `r diff(c(0,reallocation_points))`. When the methods are derived for the "non-overlapping" shrinkage estimator it is assumed that $c_i \in (0,1)$. For the "non-overlapping" estimator we do not need to make such a rigid assumption, we only need that the first value $c_1 \in (0,1)$ and the rest can be $p>n_j$ for $j=2,...,T$. 
...
(portfolios <- DOSPortfolio(data, reallocation_points, shrinkage_type = "overlapping", target_portfolio = new_target))
```
